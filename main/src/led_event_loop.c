...
// static const char* TAG = "led_event";
...
// {
//     while(1) {
//         ESP_LOGI(TAG, "application task: running application task");
...
//         loop = "loop without task";
//     else
//         loop = "loop with task";

//     ESP_LOGI(TAG, "handler %s:%s from %s, iteration %d", base, "TASK_TIERATION_EVENT", loop, iteration);
// }
...
//                 loop_to_post_to == loop ? "loop_with_task" : "loop_without_task",
...
//     ESP_ERROR_CHECK(esp_event_handler_instance_register_with(loop, TASK_EVENTS, TASK_ITERATION_EVENT, task_iteration_handler, loop, NULL));
